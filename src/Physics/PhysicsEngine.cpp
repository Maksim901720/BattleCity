...
namespace Physics {
	...
	{
        calculateTargetPosition(m_dynamicObjects, delta);
        ...
        for (auto it1 = m_dynamicObjects.begin(); it1 != m_dynamicObjects.end();) {
            ...
            for (auto it2 = ++it1; it2 != m_dynamicObjects.end(); ++it2) {
                ...
                if(!hasPositionIntersaction(pObject1, pObject1->getTargetPosition(), pObject2, pObject2->getTargetPosition()))
                    continue;

                if (hasPositionIntersaction(pObject1, pObject1->getTargetPosition(), pObject2, pObject2->getCurrentPosition())) {
                    pObject1->getTargetPosition() = pObject1->getCurrentPosition();
                }

                if (hasPositionIntersaction(pObject1, pObject1->getCurrentPosition(), pObject2, pObject2->getTargetPosition())) {
                    pObject2->getTargetPosition() = pObject2->getCurrentPosition();
                }
            }
        }

        updatePosition(m_dynamicObjects);
	}
    ...
    void PhysicsEngine::calculateTargetPosition(std::unordered_set<std::shared_ptr<IGameObject>>& dynamicObjects, const double delta)
    {
                ...
                for (const auto& currentDynamicObjectCollider : colliders)
                {
                    for (const auto& currentObjectToCheck : objectsToCheck)
                    {
                        ...
                        if (currentObjectToCheck->collides(currentDynamicObject->getObjectType()) && !collidersToCheck.empty())
                        {
                            ...
                            {
                                if (currentObjectCollider.isActive && hasCollidersIntersection(currentDynamicObjectCollider, newPosition,
                                    currentObjectCollider, currentObjectToCheck->getCurrentPosition()))
                                {
                                    hasCollision = true;
    ...
    bool PhysicsEngine::hasPositionIntersaction(const std::shared_ptr<IGameObject>& pObject1, const glm::vec2 position1,
