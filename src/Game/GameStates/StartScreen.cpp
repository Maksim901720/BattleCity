...
std::shared_ptr<RendererEngine::Sprite> getSpriteForDescription(const char description)
{
	switch (description)
	{
	case '0':
...
StartScreen::StartScreen(const std::vector<std::string>& startScreenDescription, Game* pGame)
	: m_keyReleased(true), m_pGame(pGame), m_currentMenuSelevtion(0),
	m_menuSprite(
		std::make_pair(
			ResourceManager::getSprite("menu"), 
			glm::vec2(11 * BLOCK_SIZE, STARTSCREEN_HEIGHT - startScreenDescription.size() * BLOCK_SIZE - MENU_HEIGHT - 5 * BLOCK_SIZE)
		)), 
	m_tankSprite(
		std::make_pair(
			ResourceManager::getSprite("player1_yellow_tank_type1_sprite_right"),
			glm::vec2(8 * BLOCK_SIZE, m_menuSprite.second.y + 6 * BLOCK_SIZE - m_currentMenuSelevtion * 2 * BLOCK_SIZE)
		)),
	m_tankSpriteAnimator(m_tankSprite.first)
{
	...
	auto bottomOffset = STARTSCREEN_HEIGHT - 2 * BLOCK_SIZE - 4 * BLOCK_SIZE;
...
void StartScreen::render() const
{
	...
	m_tankSprite.first->render(glm::vec2(m_tankSprite.second.x, m_tankSprite.second.y - m_currentMenuSelevtion * 2 * BLOCK_SIZE),
		glm::vec2(SIZE_TANK), 0.f, 0.f, m_tankSpriteAnimator.getCurrentFrame());
}
...
void StartScreen::processInput(std::array<bool, 349>& keys)
{
	...
	if (m_keyReleased) {
		if (keys[GLFW_KEY_W]) {
			m_keyReleased = false;
			--m_currentMenuSelevtion;
			if (m_currentMenuSelevtion < 0)
				m_currentMenuSelevtion = 2;
		}

		else if (keys[GLFW_KEY_S]) {
			m_keyReleased = false;
			++m_currentMenuSelevtion;
			if (m_currentMenuSelevtion > 2)
				m_currentMenuSelevtion = 0;
		}
	}

	if (keys[GLFW_KEY_ENTER]) {
		switch (m_currentMenuSelevtion) {
		case 0:
			m_pGame->startNewLevel(0, Game::GameMode::OnePlayer);
			break;
		case 1:
			m_pGame->startNewLevel(0, Game::GameMode::TwoPlayer);
			break;
		default:
			break;
		}
