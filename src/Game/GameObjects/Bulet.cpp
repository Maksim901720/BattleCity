...
Bullet::Bullet(const double velocity, const glm::vec2& position, const glm::vec2& size, const glm::vec2& explosionSize, const float layer)
	...
	m_isExplosion(false)
{
	auto onCollisionCallback = [&](const IGameObject& object, const Physics::ECollisionDirection collisionDirection)
		{
			setVelocity(0);
			m_isExplosion = true;
...
void Bullet::render() const
{
	if (m_isExplosion) {
		...
		case EOrientation::Top:
			m_pSprite_explosion->render(m_position - m_explosionOffset + glm::vec2(0, m_size.y / 2.f), m_explosionSize,
				m_rotation, m_layer + 0.1f, m_spriteAnimator_explosion.getCurrentFrame());
			break;
		...
		case EOrientation::Left:
			m_pSprite_explosion->render(m_position - m_explosionOffset - glm::vec2(m_size.x / 2.f, 0), m_explosionSize,
				m_rotation, m_layer + 0.1f, m_spriteAnimator_explosion.getCurrentFrame());
			break;
...
void Bullet::fire(const glm::vec2& position, const glm::vec2& direction)
{
	...
	if (m_direction.x == 0.f) {
		m_eOrientation = (m_direction.y < 0.f) ? EOrientation::Bottom : EOrientation::Top;
	}
	else {
		m_eOrientation = (m_direction.x < 0.f) ? EOrientation::Left : EOrientation::Right;
	}
	m_isActive = true;
